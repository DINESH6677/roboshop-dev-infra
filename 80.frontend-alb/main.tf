resource "aws_lb" "frontend_alb" {
  name               = "${common_suffix_name}-frontend-alb" # roboshop-dev-backend-alb
  internal           = false
  ...
  tags = merge (
    local.common_tags,
    {
        Name = "${common_suffix_name}-frontend-alb"
    }
  )
}
...
resource "aws_route53_record" "frontend_alb" {
  zone_id = var.zone_id
  name    = "roboshop-${var.environment}.${var.domain_name}" # roboshop-dev.devopswithdinesh.shop
  type    = "A"
